...
if __name__ == '__main__':
	...
	# loss = [smp.losses.DiceLoss(mode='multilabel'),smp.losses.SoftCrossEntropyLoss(smooth_factor=0.1)]
	...
	train_epoch = smp.utils.train.TrainEpoch(model, loss=loss, metrics=metrics, optimizer=optimizer,scheduler=scheduler,device=DEVICE,verbose=True)
	...
	for i in range(0, 200):
		...
		if max_score < valid_logs['fscore']:
			...
			torch.save(model, './unet_inceptionv4_best.pth')
			print('Model saved! Best:',max_score)
		print('LR:',optimizer.param_groups[0]['lr'])
		if (i+1) % 20 == 0:
			torch.save(model,'./unet_inceptionv4_ep' + str(i) + '.pth')
